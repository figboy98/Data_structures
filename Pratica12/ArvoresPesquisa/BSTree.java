...
public class BSTree<T extends Comparable<? super T>> {   
   ...
   //Valor minimo
   public T minValue(){
	   return minValue(getRoot());
   }
   private T minValue(BSTNode<T> t){
	   if( t.getRight()==null && t.getLeft()==null) return t.getValue();
	   
	   else if(t.getLeft()==null) return t.getValue();
	
	   return minValue(t.getLeft());
   }
   
   //Valor máximo
   public T maxValue(){
	   return maxValue(getRoot());
   }
   
   private T maxValue(BSTNode<T> t){
	   if(t.getRight()==null && t.getLeft()==null) return t.getValue();
	...
	//Intervalo de Valores
	 public int countBetween(T a, T b){
		 ...
		 if(a.compareTo(getRoot().getValue())<=0 && b.compareTo(getRoot().getValue())<=0) return countBetween(getRoot().getLeft(),a,b);  //Intervalo de número está só na árvore esquerda
		 if(a.compareTo(getRoot().getValue())>=0)   return countBetween(getRoot().getRight(),a,b);                               //Intervalo de números está só na árvore direita
		 else return countBetween(getRoot(), a, b);                                                              //Intervalo de números pode estar nos dois lados da árvore
		 
	 }
	 
	 private int countBetween(BSTNode<T> t,T a , T b){
		 if(t==null) return 0;
		
		 
		 if((t.getValue().compareTo(a)>=0 && t.getValue().compareTo(b)<=0)) return 1+ countBetween(t.getLeft(),a,b) + countBetween(t.getRight(),a,b); 
		 
		 
		 else return countBetween(t.getLeft(),a,b) + countBetween(t.getRight(),a,b);
		}
		
		//Verificar se é Árvore de Pesquisa
		public boolean valid(){
			return (validLeft(getRoot().getLeft(), getRoot())) && (validRight(getRoot().getRight(), getRoot()));
		}
		
		private boolean validLeft(BSTNode<T> t, BSTNode<T> aux){
			if(t==null) return true;
			
			 if((aux.getValue().compareTo(t.getValue()))<0) return false;
			
			
			if(t.getRight()!=null)
				if((t.getValue().compareTo(t.getRight().getValue()))>0) return false;
			
			
			if(t.getLeft()!=null)			

				if((t.getValue().compareTo(t.getLeft().getValue()))<0) return false;
			
			
			
			 return validLeft(t.getRight(), aux) && validLeft(t.getLeft(),aux);
		}
		
		private boolean validRight(BSTNode<T> t, BSTNode<T> aux){
			...
			if(t.getLeft()!=null)
			 ...
			 return validRight(t.getRight(), aux) && validRight(t.getLeft(),aux);
		}
